...
def main():
    logger = get_logger("stag", config_app.path.logs_folder)
    spark = (
    ...
    def foreach_batch_function(batch_df: DataFrame, batch_id: int) -> None:
        ...
        for topic in topics:
            if "test" in topic:
                logger.info(f"Start batch process event of topic: {topic}")
                ...
                logger.info(f"start raw event to clickhouse ...")
                raw_df_to_db(json_df, topic, batch_id)
                logger.info(f"end raw event to clickhouse ...")
                ...
                for row in event_names:
                    ...
                    if processor_class:
                        ...
                        processor = processor_class(spark, broadcast_vars, game_schema, logger)
                ...
                logger.info(f"End batch process event of topic: {topic}")
    ...
    query = (
        parsed_df
            ...
            .option("checkpointLocation", config_app.path.checkpoint_location_stag)
