...
class ScreenProcessor(BaseProcessor):
    ...
    def process(self, df: DataFrame) -> None:
        ...
        enriched_df = (
            ...
            .withColumn(
                "screen_session", self.get_field("screen_session", "eventData_json")
            )
            .withColumn(
                "total_duration", self.get_field("total_duration", "eventData_json")
            )
        )
        ...
        df_screen = df_fact.select(
            ...
            F.col("id_item").cast("int"),
            F.col("screen_session").cast("int"),
            F.col("total_duration").cast("int"),
            F.col("sequence_id").cast("int"),
            F.col("batch_id"),
        )

        write_to_postgres(df_screen, f"{self.schema}.fact_screen")
